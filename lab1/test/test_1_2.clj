...
(deftest n-0-empty-only
  (is (= [""]
         (seq (core-1-2/all-strings-tail symbols 0)))))

(deftest n-1-all-singletons
  (is (= #{"a" "b" "c"}
         (set (core-1-2/all-strings-tail symbols 1)))))

(deftest example-n-2
  (is (= #{"ab" "ac" "ba" "bc" "ca" "cb"}
         (set (core-1-2/all-strings-tail symbols 2)))))

(deftest property-no-adjacent
  (doseq [n (range 0 5)
          s (core-1-2/all-strings-tail symbols n)]
...
(deftest size-check
  (doseq [n (range 0 6)]
    (let [expected (if (zero? n) 1
          ...
          actual   (count (core-1-2/all-strings-tail symbols n))]
      (is (= expected actual) (str "n=" n)))))

(deftest empty-symbols
  (is (= [""]
         (seq (core-1-2/all-strings-tail [] 0))))
  (is (= []
         (vec (core-1-2/all-strings-tail [] 1)))))
